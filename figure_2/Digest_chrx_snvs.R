...
load("/n/data1/hms/dbmi/park/jluquette/pta/ad_control_neurons/metadata_combined.rda")
...
cat('Finding germline SNPs\n')
snps <- do.call(rbind, lapply(donors, function(donor) {
        d <- paste0('/n/data1/hms/dbmi/park/jluquette/pta/',
                    donor, '/scansnv_justX/snv/mmq60.tab')
        cat('reading', d, '\n')
...
cat('Finding somatic SNVs\n')
ssnvs <- do.call(rbind, lapply(donors, function(donor) {
        d <- paste0('/n/data1/hms/dbmi/park/jluquette/pta/',
                    donor, '/scansnv_justX/')
        print(d)
        g <- read.table(paste0(d, 'snv/mmq60.tab'),
                        header=T, stringsAsFactors=F, check.names=F)
        p <- read.table(paste0(d, 'snv/somatic_positions.chrX.tab'),
                        header=T, stringsAsFactors=F, check.names=F)
...
write.csv(ssnvs, file='PROTECTED_ssnvs_chrX_simple_genotyping.csv') 
write.csv(snps, file='PROTECTED_snps_chrX_simple_genotyping.csv')

tab <- do.call(rbind, lapply(split(snps, snps$sample), function(x) {
    N=sum(x$is.germline)
...
muts <- do.call(rbind, lapply(split(ssnvs, ssnvs$sample), function(x) {
    data.frame(donor=x$donor[1], sample=x$sample[1], n.somatic.snvs=sum(x$is.somatic))
}))
tab <- merge(tab, muts)
tab$somatic.burden <- tab$n.somatic.snvs/tab$sensitivity
# Excluded MDA neuron
tab <- tab[tab$sample != '5087pfc-Rp3C5',]

write.csv(tab, file='Collected_chrX_simple_genotyping_SNPs_sSNVs.csv')
